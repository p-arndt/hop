...
- **Shell tabs:** `S` in the host list — or `alt+0` from inside a focused pane — opens another shell on a host that is already connected: a second channel on the same SSH connection (no re-handshake), shown as a tab strip (`shell 1 │ shell 2`) that appears with the second shell. `alt+←/→` cycle, `alt+1..9` jump (alt+*letters* stay free: readline binds them). `exit` closes a tab; the last one exiting ends the session unless a browser/editor still holds the connection.
...
- **Vim motions + back/forward keys:** `gg`/`G`, `H`/`M`/`L`, `ctrl+d`/`ctrl+u`, `ctrl+f`/`ctrl+b` in both the host list and the browser. `enter`/`l`/`right` descends, `h`/`left` backs out; `left` at the browser's top directory pops back to hop. The terminal pane reserves `ctrl+o`, a 400 ms double-`esc`, and `left` *at a bare prompt only* (nothing typed since the last enter, no alt-screen program) — where the shell would have done nothing with it anyway. Every other key belongs to the remote shell (a lone `esc` is still forwarded). See [KEYBINDINGS.md](KEYBINDINGS.md).
