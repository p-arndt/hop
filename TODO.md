...
- [ ] **Bubble Tea v2.** v1 discards the Windows console's `ControlKeyState` and virtual key code, which is why `internal/tui/altgr.go` guesses AltGr from the *shape* of a key's runes and drops every NUL-carrying key — costing ctrl+space and ctrl+2. v2 reads input through `ultraviolet`, which names the modifiers and detects AltGr outright, so the whole file goes away. Measured plan, per file and with the traps: [docs/dev/bubbletea-v2-migration.md](docs/dev/bubbletea-v2-migration.md).
