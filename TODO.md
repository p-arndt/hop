...
- [x] **Bubble Tea v2** (`charm.land/bubbletea/v2`): v2 reads the Windows console as VT, so the console composes the character itself and a modifier press produces no event. Both Windows keyboard hacks are gone — `internal/tui/altgr.go` is deleted — and hop can send a NUL byte again (ctrl+space). The key encoder now reads `Code`+`Mod` instead of 30 key types, the mouse routing keeps its shape behind a `mouseEvt` adapter, and the alt screen and mouse mode moved into the view. Config key names are unchanged. `just ci` green; **the manual per-terminal pass is still open** — see [docs/dev/bubbletea-v2-migration.md](docs/dev/bubbletea-v2-migration.md).
