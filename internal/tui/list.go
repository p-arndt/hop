...
func (m *model) renderList(w, h int) string {
	...
	// A box grown past the sidebar height would take the layout with it.
	return style.Width(innerW).Height(h - 2).Render(fitLines(b.String(), h-2))
}

// listHasFocus is true when keys go to the host list rather than a pane or card.
...
// listRow is one drawn row: a section heading, or a host whose fi indexes m.filtered.
type listRow struct {
	// heading is empty on a host row.
	heading string
...
// listStart is the first drawn row; listRowAt runs the same arithmetic backwards.
...
// renderRow marks the cursor with an accent bar; a full-width fill nests badly with the
// styles inside the row.
