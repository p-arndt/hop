...
func vscodeModel(t *testing.T, shells int, dir string) (*model, io.Writer) {
	...
	return &model{hosts: []store.Host{{Alias: "web"}}, filtered: []int{0}, sessions: map[string]*session{"web": s}, connecting: map[string]bool{}, notify: make(chan struct{}, 1), layout: layout{width: 100, paneW: 40, paneH: 12, height: 20}, focus: focus{active: "web"}}, remote
}
